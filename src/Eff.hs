...
module Eff where
--Should hide openBracket and closeBracket
...
-- OverloadedStrings requires another type annotation here.
...
-------------------------------------------------------------------------
-- Bracketing IO effects (added by BD)
-------------------------------------------------------------------------

bracket :: (MemberU2 Lift (Lift IO) r, Member Bracket r)
        => IO a
        -> (a -> IO b)
        -> (a -> Eff r c)
        -> Eff r c
bracket before after thing = do
    ...
    openBracket $ after x
    y <- thing x
    lift $ after x -- if there's been an exception, it will
    unBracket        -- eat these lines
    return y

data Bracket v = forall a b. Bracket (IO () -> IO b) (() -> v)
    deriving (Typeable)

instance Functor (Bracket) where
    fmap f (Bracket io k) = Bracket io (f . k)

openBracket :: (MemberU2 Lift (Lift IO) r, Member Bracket r) =>
               IO a -> Eff r ()
openBracket m = send (inj . Bracket (const m))

unBracket :: (Member Bracket r, MemberU2 Lift (Lift IO) r) => 
             Eff r ()
unBracket = send (inj . Bracket (const $ return ()))

runBracket :: Eff (Bracket :> Lift IO :> Void) w
           -> Eff (Lift IO :> Void) w
runBracket m = loop (return ()) (admin m)
  where
    loop n (Val x) = lift n >> return x
    loop n (E u)   = handle_relay u (loop n) $ \(Bracket io k) -> 
                            let n' = io n >> return () in
                            loop n' (k ())
